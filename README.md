...
Add 2 binary t‍‍‍‍‍‌‍‍‌‍‌‌‌‌‌‌2‌rees. 加完之后的新tree，node的位置完全和两个input tree 重合，比如root.val = root1.val + root2.val，如果一个位置只有一个tree有node，另一个tree为空，那么就是node，两个tree都是空的位置最后也是空。follow up要我写unit test，我说assert一下，面试官说不行，要给我hint，问我知不知道serialize and deserialize tree
